...
public sealed class OriginalResizableByteWriter : Stream, IBufferWriter<byte>, IMemoryOwner<byte>
{
    private byte[]? _array;
    ...
    public OriginalResizableByteWriter() : this(ArrayPool<byte>.Shared) { }

    public OriginalResizableByteWriter(int initialCapacity = 0) : this(ArrayPool<byte>.Shared, initialCapacity) { }

    public OriginalResizableByteWriter(ArrayPool<byte> pool, int initialCapacity = 0)
    {
    ...
    {
        if (count < 0)
            throw new ArgumentOutOfRangeException(nameof(count), string.Empty);
